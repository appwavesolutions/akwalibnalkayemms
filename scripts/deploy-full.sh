...
# IP-based deployment script for VPS Kubernetes deployment
set -e

# Configuration
NAMESPACE="akwalibnalkayemms-prod"
VPS_IP="147.79.100.189"
REGISTRY="147.79.100.189:5000"
IMAGE_TAG=${1:-"latest"}
...
echo -e "${BLUE}🚀 Starting IP-based Akwalibnalkayemms Deployment${NC}"
...
# Deploy IP-based ingress
echo -e "${YELLOW}Deploying IP-based ingress...${NC}"
kubectl apply -f k8s/base/ingress-ip.yaml -n ${NAMESPACE}
...
echo -e "${GREEN}   Swagger UI: http://${VPS_IP}/swagger-ui.html${NC}"
echo -e "${GREEN}   Health Check: http://${VPS_IP}/actuator/health${NC}"
echo -e "${GREEN}   API Base: http://${VPS_IP}/api${NC}"
...
echo -e "${YELLOW}   Ingress Controller: Port 80 (HTTP)${NC}"
echo -e "${YELLOW}   Application: Port 9090 (internal)${NC}"
echo -e "${YELLOW}   MySQL: Port 3306 (internal)${NC}"
echo -e "${YELLOW}   Registry: Port 5000 (Docker registry)${NC}"
